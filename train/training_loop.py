...
class TrainLoop:
    def __init__(self, args, train_platform, model, diffusion, data, save_dir):
        self.args = args
    ...
    def run_loop(self):
        for epoch in range(self.num_epochs):
            ...
            for motion, cond in tqdm(self.data):
                ...
                self.run_step(motion, cond, self.awl)
                if self.step % self.log_interval == 0:
                    for k,v in logger.get_current().dumpkvs().items():
                        ...
                        elif k in ['step', 'samples'] or '_q' in k:
                            continue
                        else:
                            logger.log('step[{}]: {}[{:0.5f}]'.format(self.step+self.resume_step, k, v))
                            self.train_platform.report_scalar(name=k, value=v, iteration=self.step, group_name='Loss')
                    if self.awl is not None:
                        logger.log(f'awl parameters: {[i for i in self.awl.parameters()][0]}')
    ...
    def run_step(self, batch, cond, awl):
        self.forward_backward(batch, cond, awl)
        self.mp_trainer.optimize(self.opt)
        self._anneal_lr()
        self.log_step()

    def forward_backward(self, batch, cond, awl):
    ...
    def save(self):
        def save_checkpoint(params):
            ...
            with bf.BlobFile(bf.join(self.save_dir, 'ckpt', filename), "wb") as f:
                torch.save(state_dict, f)

        save_checkpoint(self.mp_trainer.master_params)

        with bf.BlobFile(
            bf.join(self.save_dir, 'ckpt', f"opt{(self.step+self.resume_step):09d}.pt"),
            "wb",
        ) as f:
