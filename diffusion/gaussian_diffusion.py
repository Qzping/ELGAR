...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.lambda_accel_rcxyz = lambda_accel_rcxyz
        self.lambda_hicl = lambda_hicl
        self.lambda_bicl = lambda_bicl
    ...
    def masked_l2(self, a, b, mask):
        ...
        non_zero_elements = torch.clamp(non_zero_elements, min=1e-8)

        mse_loss_val = loss / non_zero_elements
        # print('mse_loss_val', mse_loss_val)
        return mse_loss_val
    
    def masked_l2_expanded(self, a, b, mask):
        # assuming a.shape == b.shape == (bs, J, Jdim, seqlen)
        # assuming mask.shape == (bs, J, Jdim, seqlen)

        loss = self.l2_loss(a, b)
        
        loss = sum_flat(loss * mask.float())

        valid_elements = (mask == 1).float()

        non_zero_elements = sum_flat(valid_elements)
    ...
    def p_mean_variance(
        self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None, first_step=False, last_step=False, long=False
    ):
        """
        ...
        if long and not first_step and not last_step:
            model_output[1:, :, :, :120] = model_output[:-1, :, :, -120:]
        # model_output.shape: (bs, njoints, nfeats, nframes)
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            ...
            if self.model_mean_type == ModelMeanType.START_X:  # PRED START!
    ...
    def ddim_sample(
        ...
        # sample.shape: (bs, njoints, 6, nframes)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None, awl=None):
        """
        ...
        # cp_pos = model_kwargs['y']['cp_pos'].permute(0, 2, 1).unsqueeze(1)
        if 'cp_pos' in model_kwargs['y']:
            cp_pos = model_kwargs['y']['cp_pos'].permute(0, 2, 3, 1)
        if 'cp_info' in model_kwargs['y']:
            cp_info = model_kwargs['y']['cp_info'].permute(0, 2, 1)
        if 'used_finger' in model_kwargs['y']:
            used_finger = model_kwargs['y']['used_finger'].permute(0, 1)
        if 'instrument' in model_kwargs['y']:
            instrument = model_kwargs['y']['instrument']
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.lambda_vel_rcxyz > 0.:
                if self.data_rep == 'rot6d' and dataset.dataname in ['spd', 'humanact12', 'uestc']:
            ...
            if self.lambda_accel_rcxyz > 0.:
                if self.data_rep == 'rot6d' and dataset.dataname in ['spd']:
                    target_xyz = get_xyz(target) if target_xyz is None else target_xyz
                    model_output_xyz = get_xyz(model_output) if model_output_xyz is None else model_output_xyz
                    target_xyz_vel = (target_xyz[:, :, :, 1:] - target_xyz[:, :, :, :-1])
                    target_xyz_accel = (target_xyz_vel[:, :, :, 1:] - target_xyz_vel[:, :, :, :-1])
                    model_output_xyz_vel = (model_output_xyz[:, :, :, 1:] - model_output_xyz[:, :, :, :-1])
                    model_output_xyz_accel = (model_output_xyz_vel[:, :, :, 1:] - model_output_xyz_vel[:, :, :, :-1])
                    terms["accel_xyz_mse"] = self.masked_l2(target_xyz_accel, model_output_xyz_accel, mask[:, :, :, 2:])
            
            # # Minimize the distance between pred and gt
            # if self.lambda_interactive > 0.:
            #     if self.data_rep == 'rot6d' and dataset.dataname in ['spd']:
            #         target_xyz = get_xyz(target) if target_xyz is None else target_xyz
            #         model_output_xyz = get_xyz(model_output) if model_output_xyz is None else model_output_xyz

            #         nan_mask = ~torch.isnan(cp_pos).any(dim=2)  # (bs, 1, nframes)
            #         nan_mask_expanded = nan_mask.unsqueeze(1)
            #         combined_mask = mask & nan_mask_expanded

            #         lh_tips_index = [16, 17, 19, 18]
            #         # target_lh = target_xyz[:, :21]
            #         # model_output_lh = model_output_xyz[:, :21]
            #         target_lh = target_xyz[:, lh_tips_index]
            #         model_output_lh = model_output_xyz[:, lh_tips_index]

            #         cp_pos = torch.nan_to_num(cp_pos, 0)

            #         # distances_output = torch.norm(cp_pos - model_output_lh, dim=2)
            #         # distances_target = torch.norm(cp_pos - target_lh, dim=2)

            #         cp_pos_expanded = cp_pos.unsqueeze(1)  # (batchsize, 1, 4, 3, nframes)
            #         model_output_lh_expanded = model_output_lh.unsqueeze(2)  # (batchsize, 4, 1, 3, nframes)
            #         target_lh_expanded = target_lh.unsqueeze(2)  # (batchsize, 4, 1, 3, nframes)
                    
            #         distances_output = torch.norm(cp_pos_expanded - model_output_lh_expanded, dim=3)
            #         distances_target = torch.norm(cp_pos_expanded - target_lh_expanded, dim=3)

            #         # distances_output = distances_output.unsqueeze(2)
            #         # distances_target = distances_target.unsqueeze(2)

            #         # all_false = combined_mask.sum(dim=(-1, -2, -3)) == 0
            #         # valid_mask = ~all_false
            #         # distances_output = distances_output[valid_mask]
            #         # distances_target = distances_target[valid_mask]
            #         # combined_mask = combined_mask[valid_mask]

            #         distances_output = distances_output.float()
            #         distances_target = distances_target.float()
                    
            #         terms["interative_mse"] = self.masked_l2(distances_output, distances_target, combined_mask)

            # Minimize the distance between pred and cp
            # if self.lambda_interactive > 0.:
            #     if self.data_rep == 'rot6d' and dataset.dataname in ['spd']:
            #         target_xyz = get_xyz(target) if target_xyz is None else target_xyz
            #         model_output_xyz = get_xyz(model_output) if model_output_xyz is None else model_output_xyz
            ...
            #         nan_mask = ~torch.isnan(cp_pos).any(dim=2)  # (bs, 1, nframes)
            #         nan_mask_expanded = nan_mask.unsqueeze(1)  # (bs, 1, 1, nframes)
            #         combined_mask = mask & nan_mask_expanded

            #         cp_pos = torch.nan_to_num(cp_pos, 0)

            #         lh_tips_index = torch.tensor([16, 17, 19, 18, 0]).to(used_finger.device)  # used_finger in [0, 1, 2, 3, -1]
            #         used_finger_index = lh_tips_index[used_finger]
            #         used_finger_index_expanded = used_finger_index.unsqueeze(1).unsqueeze(2).repeat(1, 1, 3, 1)
            #         # ic(used_finger_index_expanded.shape)

            #         model_output_lh = torch.gather(model_output_xyz, dim=1, index=used_finger_index_expanded)

            #         distances_output = torch.norm(cp_pos - model_output_lh, dim=2)
            #         distances_output = distances_output.unsqueeze(1)
                    
            #         terms["interative_mse"] = self.masked_l2(distances_output, 
            #                                                  torch.zeros_like(distances_output),
            #                                                  combined_mask)

            # Hand Interactive Contact Loss
            if self.lambda_hicl > 0.:
                if self.data_rep == 'rot6d' and dataset.dataname in ['spd']:
                    target_xyz = get_xyz(target) if target_xyz is None else target_xyz
                    model_output_xyz = get_xyz(model_output) if model_output_xyz is None else model_output_xyz
                    ...
                    bs, nframes = used_finger.shape
                    device = used_finger.device

                    lh_tips_index = torch.tensor([16, 17, 19, 18, 0]).to(device)  # used_finger in [0, 1, 2, 3, -1]
                    used_finger_index = lh_tips_index[used_finger]
                    used_finger_index_expanded = used_finger_index.unsqueeze(1).unsqueeze(2).repeat(1, 1, 3, 1)
                    # ic(used_finger_index_expanded.shape)

                    model_output_used_finger = torch.gather(model_output_xyz, dim=1, index=used_finger_index_expanded)

                    distances_cp_output_used_finger = torch.norm(cp_pos - model_output_used_finger, dim=2)
                    distances_cp_output_used_finger = distances_cp_output_used_finger.unsqueeze(1)

                    lh_tips_index = lh_tips_index[:-1]
                    target_lh = target_xyz[:, lh_tips_index]
                    model_output_lh = model_output_xyz[:, lh_tips_index]
                    
                    cp_pos_expanded = cp_pos.unsqueeze(1)  # (batchsize, 1, 1, 3, nframes)
                    model_output_lh_expanded = model_output_lh.unsqueeze(2)  # (batchsize, 4, 1, 3, nframes)
                    target_lh_expanded = target_lh.unsqueeze(2)  # (batchsize, 4, 1, 3, nframes)
                    ...
                    used_finger_mask = torch.ones((bs, 4, nframes), dtype=torch.bool, device=device)
                    used_finger_mask[torch.arange(bs).unsqueeze(1), used_finger.clamp(min=0, max=3), torch.arange(nframes)] = False
                    used_finger_mask = used_finger_mask.unsqueeze(2)
                    combine_used_finger_mask = mask & used_finger_mask & nan_mask_expanded

                    contact_loss = self.masked_l2(distances_cp_output_used_finger,
                                                  torch.zeros_like(distances_cp_output_used_finger),
                                                  combined_mask)
                    
                    interactive_loss = self.masked_l2_expanded(distances_output, distances_target, combine_used_finger_mask)
                    
                    terms["hicl"] = contact_loss + interactive_loss


            # Bow Contact Loss
            # if self.lambda_bow_interactive > 0.:
            #     if self.data_rep == 'rot6d' and dataset.dataname in ['spd']:
            #         target_xyz = get_xyz(target) if target_xyz is None else target_xyz
            #         model_output_xyz = get_xyz(model_output) if model_output_xyz is None else model_output_xyz

            #         bs, _, _, seqlen = model_output_xyz.shape
            #         device = instrument.device

            #         instrument_pos = instrument[0]  # instrument is static
            #         string_points_idx = torch.tensor([[2, 4], [8, 10], [7, 9], [1, 3]], device=device)
            #         string_points = instrument_pos[string_points_idx]
            #         zero_points = torch.zeros((1, 2, 3), device=device)
            #         string_points = torch.cat((string_points, zero_points), dim=0)  # index -1 gets origin
            #         # ic(string_points.shape)
            #         string_idx = cp_info[:, 0, :]
            #         string_idx = string_idx.type(torch.int64)  # (bs, seqlen)

            #         nan_mask = string_idx != -1  # (bs, nframes)
            #         nan_mask_expanded = nan_mask.unsqueeze(1).unsqueeze(2)
            #         combined_mask = mask & nan_mask_expanded

            #         extracted_points = string_points[string_idx, :, :]
            #         extracted_points = extracted_points.permute(0, 2, 3, 1)
            #         # ic(extracted_points.shape)

            #         string_start = extracted_points[:, 0]
            #         string_end = extracted_points[:, 1]

            #         bow_frog = model_output_xyz[:, 42]
            #         bow_tip = model_output_xyz[:, 43]

            #         v1 = (string_end - string_start).float()  # (bs, 3, seqlen)
            #         v2 = (bow_tip - bow_frog).float()  # (bs, 3, seqlen)

            #         diff = bow_tip - string_end

            #         cross_product = torch.cross(v1, v2, dim=1)
            #         cross_product_norm = torch.norm(cross_product, dim=1)
            #         cross_product_norm = torch.where(cross_product_norm == 0, torch.tensor(float('inf')), cross_product_norm)
            #         dot_product = torch.sum(diff * cross_product, dim=1)
            #         distance = torch.abs(dot_product) / cross_product_norm
                    
            #         distance = distance.unsqueeze(1).unsqueeze(2)
            #         # ic(distance.shape)

            #         terms["bow_interative_mse"] = self.masked_l2(distance, 
            #                                                  torch.zeros_like(distance),
            #                                                  mask)

            # Bow Interactive Contact Loss
            if self.lambda_bicl > 0.:
                if self.data_rep == 'rot6d' and dataset.dataname in ['spd']:
                    target_xyz = get_xyz(target) if target_xyz is None else target_xyz
                    model_output_xyz = get_xyz(model_output) if model_output_xyz is None else model_output_xyz
                    ...
                    cross_product_norm = torch.where(cross_product_norm == 0, torch.tensor(float('inf')), cross_product_norm)
                    dot_product = torch.sum(diff * cross_product, dim=1)
                    bow_string_distance = torch.abs(dot_product) / cross_product_norm
                    
                    bow_string_distance = bow_string_distance.unsqueeze(1).unsqueeze(2)
                    # ic(distance.shape)

                    bow_contact_loss = self.masked_l2(bow_string_distance,
                                                      torch.zeros_like(bow_string_distance),
                                                      combined_mask)
                    

                    bow_start_idx = 42
                    target_bow_start = target_xyz[:, bow_start_idx]
                    output_bow_start = model_output_xyz[:, bow_start_idx]

                    d = string_end - string_start
                    v_target = target_bow_start - string_start
                    v_output = output_bow_start - string_start

                    d_dot_d = torch.sum(d**2, dim=1, keepdim=True)
                    nan_mask_dot_expanded = nan_mask.unsqueeze(1)
                    d_dot_d[~nan_mask_dot_expanded] = 1e-8  # avoid divide zero
                    
                    v_proj_target = torch.sum(v_target * d, dim=1, keepdim=True) / d_dot_d * d
                    v_proj_output = torch.sum(v_output * d, dim=1, keepdim=True) / d_dot_d * d

                    bow_start_string_distance_target = torch.norm(v_target - v_proj_target, dim=1)
                    bow_start_string_distance_output = torch.norm(v_output - v_proj_output, dim=1)

                    bow_start_string_distance_target = bow_start_string_distance_target.unsqueeze(1).unsqueeze(2)
                    bow_start_string_distance_output = bow_start_string_distance_output.unsqueeze(1).unsqueeze(2)

                    bow_start_string_interactive_loss = self.masked_l2(bow_start_string_distance_target, bow_start_string_distance_output, combined_mask)


                    bow_end_idx = 43
                    target_bow_end = target_xyz[:, bow_end_idx]
                    output_bow_end = model_output_xyz[:, bow_end_idx]

                    d = string_end - string_start
                    v_target = target_bow_end - string_start
                    v_output = output_bow_end - string_start
                    ...
                    bow_end_string_distance_target = torch.norm(v_target - v_proj_target, dim=1)
                    bow_end_string_distance_output = torch.norm(v_output - v_proj_output, dim=1)

                    bow_end_string_distance_target = bow_end_string_distance_target.unsqueeze(1).unsqueeze(2)
                    bow_end_string_distance_output = bow_end_string_distance_output.unsqueeze(1).unsqueeze(2)

                    bow_end_string_interactive_loss = self.masked_l2(bow_end_string_distance_target, bow_end_string_distance_output, combined_mask)

                    terms["bicl"] = bow_contact_loss + bow_start_string_interactive_loss + bow_end_string_interactive_loss


            # if self.lambda_rh_interactive > 0.:
            #     if self.data_rep == 'rot6d' and dataset.dataname in ['spd']:
            #         target_xyz = get_xyz(target) if target_xyz is None else target_xyz
            #         model_output_xyz = get_xyz(model_output) if model_output_xyz is None else model_output_xyz
            ...
            #         distance_target = torch.norm(v_target - v_proj_target, dim=1)
            #         distance_output = torch.norm(v_output - v_proj_output, dim=1)

            #         distance_target = distance_target.unsqueeze(1).unsqueeze(2)
            #         distance_output = distance_output.unsqueeze(1).unsqueeze(2)

            #         terms["rh_interactive_mse"] = self.masked_l2(distance_target, distance_output, combined_mask)


            # if self.lambda_bowing > 0.:
            #     if self.data_rep == 'rot6d' and dataset.dataname in ['spd']:
            #         target_xyz = get_xyz(target) if target_xyz is None else target_xyz
            #         model_output_xyz = get_xyz(model_output) if model_output_xyz is None else model_output_xyz
            #         rh_wrist_idx, bow_start_idx, bow_end_idx = -3, -2, -1
            #         target_xyz_vel = (target_xyz[:, :, :, 1:] - target_xyz[:, :, :, :-1])
            #         model_output_xyz_vel = (model_output_xyz[:, :, :, 1:] - model_output_xyz[:, :, :, :-1])
            #         terms["bowing"] = self.masked_l2(target_xyz_vel, model_output_xyz_vel, mask[:, :, :, 1:])
            ...
            if self.lambda_fc > 0.:
                ...
                if self.data_rep == 'rot6d':
                    ...
                    l_ankle_idx, r_ankle_idx, l_foot_idx, r_foot_idx = 44+7, 44+8, 44+10, 44+11
            ...
            if self.lambda_vel > 0.:
                ...
                # print('target_vel:', target_vel[:, :-1, :, :])
                # print('output_vel:', model_output_vel[:, :-1, :, :])
            ...
            #                 (self.lambda_accel_rcxyz * terms.get('accel_xyz_mse', 0.)) +\
            #                 (self.lambda_rh_interactive * terms.get("rh_interactive_mse", 0.)) +\
            #                 (self.lambda_interactive * terms.get('interative_mse', 0.)) +\
            #                 (self.lambda_bow_interactive * terms.get('bow_interative_mse', 0.))
            ...
            if awl is not None:
                ...
                #             (self.lambda_accel_rcxyz * terms.get('accel_xyz_mse', 0.)) +\
                #             (self.lambda_rh_interactive * terms.get("rh_interactive_mse", 0.)) +\
                #             (awl(terms.get('interative_mse', 0.) + terms.get('bow_interative_mse', 0.)))

                terms["loss"] = (awl(terms["rot_mse"] +
                                    terms.get('fc', 0.) + 
                                    terms.get('vel_mse', 0.) +
                ...
                terms["loss"] = terms["rot_mse"] + terms.get('vb', 0.) +\
                            ...
                            (self.lambda_vel_rcxyz * terms.get('vel_xyz_mse', 0.)) +\
                            (self.lambda_accel_rcxyz * terms.get('accel_xyz_mse', 0.)) +\
                            (self.lambda_hicl * terms.get('hicl', 0.)) +\
                            (self.lambda_bicl * terms.get('bicl', 0.))
