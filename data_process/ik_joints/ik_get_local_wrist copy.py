...
def elbow_obj_func(elbow_rot, smplx_model, pose_body, betas, trans, root_orient, wrist_target):
    ...
    # ic(wrist_result)
    # ic(wrist_target)
    # dist = np.linalg.norm(wrist_target - wrist_result)
...
if __name__ == '__main__':
    ...
    J_original = output.joints[:, :22].detach().numpy()  # body
    ...
    for i in range(frame_num):
        print(f'frame {i} ik...')
        elbow_rot_vec = pose_body[i, -12:-6].numpy().copy()
        pose_body_frame = pose_body[i].numpy().copy()
        elbow_result = minimize(elbow_obj_func, elbow_rot_vec,
                                args=(model, pose_body_frame, 
                                      betas[i].unsqueeze(0), 
                                      trans[i].unsqueeze(0),
                                      root_orient[i].unsqueeze(0),
                                      J_original[i, -2:]),
                                method='L-BFGS-B',
                                # method='Powell',
                                options={'ftol': 1e-8, 'gtol': 1e-8, 'disp': True, 'maxiter': 50})
        print(f'frame {i} done.')
        pose_body[i, -12:-6] = torch.tensor(elbow_result.x, dtype=torch.float)
    ...
    '''
    root_orient = axis_angle_to_matrix(root_orient)  # (bs, 3, 3)
    pose_body = axis_angle_to_matrix(pose_body)  # (bs, 21, 3, 3)

    lh_wrist_global = lh_pose.reshape(batch_size, 16, 6)[:,0,:]
    lh_wrist_global = rotation_6d_to_matrix(lh_wrist_global)  # (bs, 3, 3)

    R_index = [2, 5, 8, 12, 15, 17]
    R_accumulate = root_orient.clone()

    for id in R_index:
        R_accumulate = torch.matmul(R_accumulate[:],pose_body[:, id])
    
    lh_wrist_local = torch.matmul(torch.linalg.inv(R_accumulate), lh_wrist_global)


    pose_body[:,19] = lh_wrist_local
    pose_body = matrix_to_axis_angle(pose_body).reshape(-1, 63)
    ic(pose_body.shape)
