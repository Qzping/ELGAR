...
class BodyModel(nn.Module):
    ...
    def __init__(self,
        ...
        if bm_fname.endswith('.npz'):
            ...
            with open(bm_fname, 'rb') as smpl_file:
                smpl_dict = pickle.load(smpl_file, encoding='latin1')
        else:
            raise ValueError(f'bm_fname must be a .npz / .pkl file: {bm_fname}')
        ...
        # smplh / smplx
        self.vertex_joint_selector = VertexJointSelector(vertex_ids=VERTEX_IDS[self.model_type])

        #print(self.vertex_joint_selector)
        self.use_dmpl = False
        ...
        if use_posedirs:
            ...
            print('posedirs.shape:',posedirs.shape)
    ...
    def forward(self, root_orient=None, pose_body=None, pose_hand=None, pose_jaw=None, pose_eye=None, betas=None,
        ...
        if self.model_type in ['smplh', 'smpl']:
            ...
            #else:
                #print(1)
                #full_pose = torch.cat([root_orient, pose_body.unsqueeze(1), pose_jaw, pose_eye, pose_hand], dim=-1)  # orient:3, body:63, jaw:3, eyel:3, eyer:3, handl, handr
        ...
        verts, Jtr = lbs(betas=shape_components, pose=full_pose, v_template=v_template,
                            ...
                            dtype=self.dtype,batch_size=batch_size)
